...
namespace TouchPanels.Devices
{
	...
	public sealed class Tsc2046 : ITouchDevice
	{
        ...
        void ITouchDevice.ReadTouchpoints()
        {
            ...
            {
                //get X data
                writeBuffer24[0] = (CMD_START | CMD_12BIT | CMD_DIFF | CMD_X_POS);
                touchSPI.TransferFullDuplex(writeBuffer24, readBuffer24);
                a2 = readBuffer24[1]; 
                b2 = readBuffer24[2]; 
                x = ((a2 << 4) | (b2 >> 4)); //12bit: ((a<<4)|(b>>4))
                //get Y data
                writeBuffer24[0] = (CMD_START | CMD_12BIT | CMD_DIFF | CMD_Y_POS);
                touchSPI.TransferFullDuplex(writeBuffer24, readBuffer24);
                a2 = readBuffer24[1];
                b2 = readBuffer24[2];
                y = ((a2 << 4) | (b2 >> 4)); //12bit: ((a<<4)|(b>>4))
                if (x > 0 && y > 0)
                {
